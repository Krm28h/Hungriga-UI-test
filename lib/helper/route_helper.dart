...
import 'package:stackfood_multivendor/features/ai_chat_bot/screens/ai_chat_bot_screen.dart';
import 'package:stackfood_multivendor/features/ai_chat_bot/screens/ai_chat_details_screen.dart';
...
class RouteHelper {
  ...
  static String getAiChatDetailsScreen({int? conversationId, String? title, String? initialMessage, int? messagesCount}) {
    String encodedTitle = (title != null && title.isNotEmpty) ? base64Encode(utf8.encode(title)) : 'null';
    String encodedMessage = (initialMessage != null && initialMessage.isNotEmpty) ? base64Encode(utf8.encode(initialMessage)) : 'null';
    return '$aiChatDetails?conversation_id=${conversationId ?? 'null'}&title=$encodedTitle&initial_message=$encodedMessage&messages_count=${messagesCount ?? 'null'}';
  }
  ...
  static List<GetPage> routes = [
    ...
    GetPage(name: aiChatBot, page: () => getRoute(const AiChatBotScreen())),
    GetPage(name: aiChatDetails, page: () {
      String? title;
      if(Get.parameters['title'] != null && Get.parameters['title'] != 'null') {
        title = utf8.decode(base64Decode(Get.parameters['title']!.replaceAll(' ', '+')));
      }
      String? initialMessage;
      if(Get.parameters['initial_message'] != null && Get.parameters['initial_message'] != 'null') {
        initialMessage = utf8.decode(base64Decode(Get.parameters['initial_message']!.replaceAll(' ', '+')));
      }
      return getRoute(AiChatDetailsScreen(
        ...
        messagesCount: (Get.parameters['messages_count'] != null && Get.parameters['messages_count'] != 'null') ? int.tryParse(Get.parameters['messages_count']!) : null,
      ));
    }),
  ];
