...
import 'package:stackfood_multivendor/features/ai_chat_bot/widgets/ai_chat_bot_desktop_popup_widget.dart';
...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return GetBuilder<HomeController>(builder: (homeController) {
      return GetBuilder<LocalizationController>(builder: (localizationController) {
        return Scaffold(
          ...
          floatingActionButton: _buildFloatingButtons(context, homeController),
  ...
  Widget? _buildFloatingButtons(BuildContext context, HomeController homeController) {
    final bool isDesktop = ResponsiveHelper.isDesktop(context);
    final bool aiEnabled = (isDesktop ? true : homeController.showFavButton) && (_configModel?.aiChatStatus ?? false);
    final bool cashBackEnabled = (isDesktop ? true : homeController.showFavButton) && AuthHelper.isLoggedIn()
        && homeController.cashBackOfferList != null && homeController.cashBackOfferList!.isNotEmpty;

    final List<Widget> buttons = [];

    // AI on top, cashback below it (closer to the corner).
    if (aiEnabled) {
      buttons.add(isDesktop ? Padding(
        padding: EdgeInsets.only(right: isDesktop ? 20 : 0),
        child: const AiChatBotDesktopPopupWidget(),
      ) : const AiChatBotFloatingButtonWidget());
    }
    if (cashBackEnabled) {
      buttons.add(Padding(
        padding: EdgeInsets.only(bottom: isDesktop ? 50 : 0, right: isDesktop ? 20 : 0),
        child: InkWell(
    ...
    return Column(
      key: const ValueKey('home_floating_buttons'),
