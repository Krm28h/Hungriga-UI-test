...
class AiChatMessageBubbleWidget extends StatelessWidget {
  ...
  /// Lays out the assistant's lightly-marked-down reply line by line: bullet
  /// lines ("- ..." / "* ...") get a real `•` marker with a hanging indent,
  /// blank lines become spacing between paragraphs, and every line still renders
  /// inline `**bold**` emphasis. Kept dependency-free (no full markdown parser).
  Widget _buildMessageContent(String content, TextStyle baseStyle) {
    ...
    for (final String rawLine in lines) {
      ...
      // Blank line → a small gap between paragraphs/sections.
      if (trimmed.isEmpty) {
        children.add(const SizedBox(height: Dimensions.paddingSizeExtraSmall));
        continue;
      }
      ...
      if (bullet != null) {
        children.add(Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(
              padding: const EdgeInsets.only(top: 1, right: Dimensions.paddingSizeExtraSmall),
  ...
  /// Renders inline `**bold**` markdown emphasis within a single line as real
  /// bold spans (e.g. "choose one: **a** or **gs**"); other text is verbatim.
