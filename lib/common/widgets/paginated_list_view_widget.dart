...
class PaginatedListViewWidget extends StatefulWidget {
  ...
  const PaginatedListViewWidget({super.key, required this.scrollController, required this.onPaginate, required this.totalSize,
    required this.offset, required this.productView, this.enabledPagination = true, this.reverse = false, this.forceMobileStyle = false,
  });
...
class _PaginatedListViewWidgetState extends State<PaginatedListViewWidget> {
  ...
  void initState() {
    ...
    widget.scrollController.addListener(() {
      if (widget.scrollController.position.pixels == widget.scrollController.position.maxScrollExtent
        ...
        if(mounted && (widget.forceMobileStyle || !ResponsiveHelper.isDesktop(context))) {
          _paginate();
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    final bool isDesktopStyle = ResponsiveHelper.isDesktop(context) && !widget.forceMobileStyle;
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        (isDesktopStyle && (widget.totalSize == null || _offset! >= (widget.totalSize! / 10).ceil() || _offsetList.contains(_offset!+1))) ? const SizedBox() : Center(child: Padding(
          padding: (_isLoading || isDesktopStyle) ? const EdgeInsets.all(Dimensions.paddingSizeSmall) : EdgeInsets.zero,
          child: _isLoading ? const CircularProgressIndicator() : (isDesktopStyle && widget.totalSize != null) ? InkWell(
            onTap: _paginate,
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: Dimensions.paddingSizeSmall, horizontal: Dimensions.paddingSizeLarge),
              margin: isDesktopStyle ? const EdgeInsets.only(top: Dimensions.paddingSizeSmall) : null,
